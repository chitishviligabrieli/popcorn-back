...
import { JustwatchEntity } from './entities/justwatch.entity';
...
export class JustWatchService {
  private readonly logger = new Logger(JustWatchService.name);
  private url = 'https://apis.justwatch.com/graphql';
  private interval = 5000;
  private headers = {
    'Content-Type': 'application/json',
    'device-id': 'wqjDvjTCoG0oTMKOCoNKPz',
  };
  private offset = 0;
  private limit = 40;
  private number = 1;
  ...
  constructor(
    @InjectRepository(JustwatchEntity)
    ...
    for (const movie of movies) {
      ...
      while (attempt < maxAttempts) {
        try {
          ...
          const r = await axios.post(this.url, payload);
          ...
          console.error(`Request failed on attempt ${attempt + 1} for ${movie.primaryTitle}:`, error.message);
        }
